...
insert' m @ (Leaf k _) (k', v') = case compare k k' of
    EQ -> Left m'
    LT -> Right (m, m')
    GT -> Right (m', m)
    where m' = Leaf k' v'
insert' (Node2 _ ml mr) kv @ (k, _)
    | k < firstKey mr           = case insert' ml kv of
        Left ml'         -> Left $ node2 ml' mr
        Right (mll, mlr) -> Left $ node3 mll mlr mr
    | otherwise                 = case insert' mr kv of
        Left mr'         -> Left $ node2 ml mr'
        Right (mrl, mrr) -> Left $ node3 ml mrl mrr
insert' (Node3 _ ml mc mr) kv @ (k, _)
    | k < firstKey mc           = case insert' ml kv of
        Left ml'         -> Left $ node3 ml' mc mr
        Right (mll, mlr) -> Right (node2 mll mlr, node2 mc mr)
    | k < firstKey mr           = case insert' mc kv of
